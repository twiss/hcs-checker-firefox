...
HCSChecker.prototype = {
		...
		if(aTopic === 'http-on-examine-response') {
			...
			if(uri.scheme === 'https') {
				...
				if(win) {
					...
					var hashes = this.getHashesInCertificate(certificate);
					var loadedCertificate = !!certificate;
					var _this = this;
					if(!loadedCertificate) { // First request to a website.
						setTimeout(function() {
							certificate = _this.getCertificate(win);
							hashes = _this.getHashesInCertificate(certificate);
	...
	isResponseValid: function(oHttp, hashes, responseData) {
		...
		var [algo, hash] = hashes[path].split('-');
		return this.algorithmSupported(algo) && this.computeHash(algo, responseData) === hash;
	},
							...
							while(extensionsEnumerator.hasMoreElements()) {
								...
								if(extension.displayName === 'Object Identifier (2 5 29 66)') {
									...
									decoded.sub.forEach(function(hashElm) {
										hashes[hashElm.sub[0].content()] = hashElm.sub[1].content();
									});
